...
async function onSubmit(event: FormEvent<HTMLFormElement>){
    ...
    console.log("citiname",searchTerm);
    
    const response = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${searchTerm}&units=imperial&appid=${apiKey}`);
...
<>
      ...
      {/* user input code */}
      <input value={searchTerm} onChange={handleChange}type="search" id="default-search" className="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for Cities" required />
      </div>
